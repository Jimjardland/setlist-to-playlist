...
async function init (code) {
  ...
  const { accessToken, refreshToken } = await auth.getTokens(code)
...
async function createPlaylist (user, name) {
  return spotifyApi.createPlaylist(user, name, { 'public' : false })
    .then(({body}) => body)
    .catch(error => { throw new Error(error) })
}
...
async function createPlaylistFromArtist (artistName, code) {
  await init(code)
