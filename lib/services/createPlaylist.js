...
async function searchArtist (artistName) {
  return fetch(`https://api.setlist.fm/rest/1.0/search/artists?artistName=${encodeURIComponent(artistName)}&p=1&sort=relevance`, myInit)
    .then(res => res.json())
    .then(({ artist }) => {
      const artistWithTmid = artist.filter(x => { if(x.tmid) return x.mbid })
      return (artistWithTmid.length ? artistWithTmid : artist.filter(x => x.mbid))
    })
...
async function createPlaylist (user, name) {
  return spotifyApi.createPlaylist(user, `${toTitleCase(name)} - setlisttoplaylist.com`, { 'public' : false })
    .then(({body}) => body)
    .catch(error => { throw new Error(error) })
}
